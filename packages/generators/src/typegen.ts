...
export class TypeGenerator implements Generator {
  ...
  async generate(
    ...
    let output = "// Auto-generated by Blazing CMS — do not edit\n\n";

    for (const collection of collections) {
      const name = safeName(collection.slug);
    ...
    for (const global of globals) {
      const name = safeName(global.slug);
      output += `export interface ${name} {\n`;
...
function safeName(slug: string): string {
  ...
  return GLOBALS.has(name) ? `${name}Entry` : name;
}
